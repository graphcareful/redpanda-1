...
from rptest.services.native_kafka_consumer import NativeKafkaConsumer
from rptest.services.native_kafka_producer import NativeKafkaProducer
...
class WasmTest(RedpandaTest):
    def __init__(self, test_context, extra_rp_conf):
        ...
        super(WasmTest, self).__init__(test_context=test_context,
                                       extra_rp_conf=wasm_opts)
        self._context = self.redpanda._context
    ...
    def _start(self, topic_spec, scripts):
        ...
        # Wait on results from consumption of input & output topics
        producers = []
        for script in scripts:
            for topic, producer_opts in script.inputs:
                num_records, records_size = producer_opts
                producer = NativeKafkaProducer(self._context, self.redpanda,
                                               topic, num_records,
                                               records_size)
                producer.start()
                producers.append(producer)

        input_consumer = NativeKafkaConsumer(self._context, self.redpanda,
                                             input_tps, total_inputs)
        output_consumer = NativeKafkaConsumer(self._context, self.redpanda,
        ...
        self.assert_test_results(input_consumer.results,
                                 output_consumer.results)

    def wasm_test_plan(self):
        """
        Returns a list of WasmScripts to be used in the test
        """
    ...
    @cluster(num_nodes=6)
    def execute_wasm_test(self):
        """
        This method is invoked automatically by ducktape
        """
        self._start(self.topics, self.wasm_test_plan())
