...
using rrl = token_bucket_rate_tracker::recommend_rate_limit;
...
SEASTAR_THREAD_TEST_CASE(token_bucket_algorithm_test) {
    ...
    BOOST_CHECK_EQUAL(should_delay, rrl::yes);
...
SEASTAR_THREAD_TEST_CASE(token_bucket_edge_case_test) {
    ...
    token_bucket_rate_tracker rate(max_val, max_val, 2s);
