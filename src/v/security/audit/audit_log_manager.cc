...
class internal_topic_creation_failure : public std::runtime_error {
public:
    explicit internal_topic_creation_failure(const std::string& s)
      : std::runtime_error(s) {}
};
...
ss::future<> audit_client::configure() {
    try {
        ...
        _client.config().retries.set_value(std::numeric_limits<size_t>::max());
        vlog(seclog.info, "Audit log client initialized");
    } catch (...) {
        vlog(
          seclog.warn,
...
ss::future<> audit_client::mitigate_error(std::exception_ptr eptr) {
    ...
    vlog(seclog.trace, "mitigate_error: {}", eptr);
    auto f = ss::now();
    try {
...
ss::future<> audit_client::do_inform(model::node_id id) {
    ...
    vlog(seclog.info, "Informed: broker: {}, ec: {}", id, ec);
}
...
ss::future<> audit_client::create_internal_topic() {
    ...
    kafka::creatable_topic audit_topic{
      ...
      .configs = {
        ...
        kafka::createable_topic_config{
          ...
          .value{seven_days}}}};
    vlog(
      seclog.info, "Creating audit log topic with settings: {}", audit_topic);
    ...
    if (topic.error_code == kafka::error_code::none) {
        vlog(seclog.debug, "Auditing: created audit log topic: {}", topic);
    } else if (topic.error_code == kafka::error_code::topic_already_exists) {
        vlog(seclog.debug, "Auditing: topic already exists");
        ...
        const auto msg = topic.error_message.has_value() ? *topic.error_message
                                                         : "<no_err_msg>";
        throw internal_topic_creation_failure(
...
ss::future<> audit_client::shutdown() {
    ...
    vlog(seclog.info, "Shutting down audit client");
    _as.request_abort();
    _send_sem.broken();
              ...
              return _client
                ...
                .handle_exception_type(
                  [](const kafka::client::partition_error& ex) {
                      ...
                      vlog(
                        seclog.error, "Audit records dropped, reason: {}", ex);
                  })
...
ss::future<> audit_sink::do_toggle(bool enabled) {
    try {
        ...
        vlog(seclog.info, "Failed to toggle audit status, shutting down");
    } catch (...) {
        vlog(
          seclog.error,
...
ss::future<> audit_log_manager::start() {
    ...
    _audit_enabled.watch([this] {
        try {
            ...
            vlog(
              seclog.debug, "Failed to toggle auditing state, shutting down");
        } catch (...) {
            vlog(
              seclog.error,
    ...
    if (_audit_enabled()) {
        vlog(seclog.info, "Starting audit_log_manager");
...
ss::future<> audit_log_manager::stop() {
    ...
    _as.request_abort();
    if (ss::this_shard_id() == audit_client::shard_id) {
        vlog(seclog.info, "Shutting down audit log manager");
        co_await _sink->stop();
    }
    if (!_gate.is_closed()) {
