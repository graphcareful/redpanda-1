...
public:
    ss::future<ss::stop_iteration> operator()(const model::record_batch& rb) {
        ...
        if (rb.compressed()) {
            f = storage::internal::decompress_batch(std::move(rb))
                  .then([this](model::record_batch rb) {
                      return verify_records(rb);
                  });
        }
...
ss::future<> event_publisher::create_coproc_internal_topic() {
    return _client
      ...
      .then([](kafka::create_topics_response response) {
          ...
          vassert(
            response.data.topics[0].name == internal_tp.topic,
    ...
    return std::move(reader)
      .for_each_ref(
        coproc::reference_window_consumer(
          ...
          coproc::wasm::publisher(_client, internal_tp)),
        model::no_timeout)
      .then([](auto three_tuple) {
          vassert(std::get<0>(three_tuple) == true, "Batch crc checks failed");
          return std::make_tuple(
            std::get<1>(three_tuple), std::move(std::get<2>(three_tuple)));
      });
}
