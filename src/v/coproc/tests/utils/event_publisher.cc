...
event_publisher::event_publisher(kafka::client::client& client) noexcept
  : _client(client) {}
...
ss::future<> event_publisher::start() {
    ...
    return create_coproc_internal_topic().then(
      [this] { return _client.update_metadata(); });
}
      ...
      .then([this](model::record_batch_reader rbr) {
          return std::move(rbr)
            ...
            .then([](auto tuple) {
                ...
                for (auto& rp : r) {
                    vassert(rp.error_code == kafka::error_code::none, "ERROR");
                }
