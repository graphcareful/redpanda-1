...
FIXTURE_TEST(test_coproc_router_multi_route, coproc_bench_fixture) {
    ...
    copro_map mapping = {
      {input_one, {even_mt_one, odd_mt_one}},
      {input_two, {even_mt_two, odd_mt_two}}};
    setup(inputs).get();
    ...
    /// Run the test
    auto drain_results = start_benchmark(
                           build_simple_opts(
                             std::move(inputs), std::move(mapping), 50, 25))
                           .get0();
...
FIXTURE_TEST(test_coproc_router_giant_fanin, coproc_bench_fixture) {
    ...
    copro_map mapping = {{source_topic, {output_topic}}};
    setup(inputs).get();
    enable_coprocessors(std::move(deploys)).get();

    auto drain_results = start_benchmark(
                           build_simple_opts(
                             std::move(inputs), std::move(mapping), 10, 500))
                           .get0();
...
FIXTURE_TEST(test_coproc_router_giant_one_to_many, coproc_bench_fixture) {
    ...
    log_layout_map inputs = {{source_topic, n_partitions}};
    copro_map mapping = {{source_topic, {}}};
    for (std::size_t i = 0; i < n_copros; ++i) {
        ...
        mapping.begin()->second.emplace_back(materialized_topic);
    }
    setup(inputs).get();
    enable_coprocessors(std::move(deploys)).get();
    ...
    auto drain_results = start_benchmark(
                           build_simple_opts(
                             std::move(inputs), std::move(mapping), 10, 10))
                           .get0();
