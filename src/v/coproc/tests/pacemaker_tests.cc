...
//                               bar,
//                               coproc::topic_ingestion_policy::stored)}}},
//                          {.id = 7777,
//                           .data{
//                             .tid = copro_typeid::null_coprocessor,
//                             .topics = {std::make_pair<>(
...
//     model::ntp input_ntp(model::kafka_namespace, foo,
//     model::partition_id(0)); produce(input_ntp, make_random_batch(40)).get();
...
//       .get();
//     auto fn =
//       [this, input_ntp, output_ntp](model::offset start) ->
//       ss::future<size_t> {
...
FIXTURE_TEST(test_copro_spam_delete, coproc_test_fixture) {
    const model::topic_namespace tn(
      model::kafka_namespace, model::topic("smap_delete"));
    const int n_partitions = 6;
    const coproc::script_id id(192837);
    setup({{tn.tp, n_partitions}}).get();
    // Register a coprocessor that throws
    enable_coprocessors(
      {{.id = id(),
        .data{
          .tid = copro_typeid::identity_coprocessor,
          .topics = {{tn.tp, coproc::topic_ingestion_policy::earliest}}}}})
      .get();

    // Push some data across input topic....
    std::vector<ss::future<>> fs;
    fs.reserve(n_partitions);
    for (auto i = 0; i < n_partitions; ++i) {
        fs.emplace_back(produce(
          model::ntp(tn.ns, tn.tp, model::partition_id(i)),
          make_random_batch(1000)));
    }

    /// Delete the materialized topic once every 100ms, triggering some chaos in
    /// the system. Coproc should re-create the log and start from earliest
    /// offset as configured above.
    model::topic_namespace output_tn(
      tn.ns,
      to_materialized_topic(tn.tp, identity_coprocessor::identity_topic));
    for (auto i = 0; i < 3; ++i) {
        root_fixture()->delete_topic(output_tn).get();
        ss::sleep(100ms).get();
    }

    ss::when_all_succeed(fs.begin(), fs.end()).get();

    const bool exists = root_fixture()->app.metadata_cache.local().contains(
      output_tn, model::partition_id(0));
    if (exists) {
        /// If by tests end topic exists (because of interleaving of calls),
        /// ensure it has all expected data
        info("Data existed by tests end");
        try {
            for (auto i = 0; i < n_partitions; ++i) {
                auto recs
                  = consume(
                      model::ntp(
                        output_tn.ns, output_tn.tp, model::partition_id(i)),
                      1000)
                      .get();
                BOOST_CHECK_EQUAL(num_records(recs), 1000);
            }
        } catch (...) {
            /// If the above times out, ensure it was because the async deletion
            /// request finally completed
            const bool exists
              = root_fixture()->app.metadata_cache.local().contains(
                output_tn, model::partition_id(0));
            BOOST_CHECK(exists);
        }
    }
