...
class coproc_fixture_base {
public:
    using opt_reader_data_t = std::optional<model::record_batch_reader::data_t>;

    explicit coproc_fixture_base(bool start_router)
      : _start_router(start_router) {}
    coproc_fixture_base(coproc_fixture_base&) = delete;
    coproc_fixture_base(coproc_fixture_base&&) = delete;

    virtual ~coproc_fixture_base();
...
protected:
    ...
    void startup(log_layout_map&& data);
