...
#include "coproc/tests/fixtures/redpanda_with_copro_fixture.h"
...
ss::future<model::timeout_clock::duration> timed_delete_partition(
  ...
  coproc::partition_manager& local,
  model::ntp tp2_ntp) {
...
FIXTURE_TEST(test_coproc_partition_shutdown, redpanda_with_copro_fixture) {
    ...
    add_topic(to_tpns(tp_ntp)).get();
    add_non_replicable_topic(to_tpns(tp_ntp), to_tpns(tp2_ntp)).get();

    /// Wait until the partitions are created
    std::optional<ss::shard_id> home_shard;
    tests::cooperative_spin_wait_with_timeout(
      3s,
      [this, tp2_ntp, &home_shard]() mutable {
          home_shard = app.shard_table.local().shard_for(tp2_ntp);
    ...
    auto& pm = app.coprocessing->get_partition_manager();
    auto delete_time = pm.invoke_on(
                           *home_shard,
                           [this, tp2_ntp](coproc::partition_manager& local) {
    ...
    bool exists = partition_exists(pm, *home_shard, tp2_ntp).get();
