...
new_coproc_test_fixture::new_coproc_test_fixture()
...
ss::future<>
new_coproc_test_fixture::enable_coprocessors(std::vector<deploy> copros) {
...
ss::future<>
new_coproc_test_fixture::disable_coprocessors(std::vector<uint64_t> ids) {
    std::vector<coproc::wasm::event> events;
...
ss::future<> new_coproc_test_fixture::setup(log_layout_map llm) {
...
new_coproc_test_fixture::produce(
  model::ntp ntp, model::record_batch_reader rbr) {
...
ss::future<> new_coproc_test_fixture::wait_for_materialized(
...
ss::future<ss::stop_iteration> new_coproc_test_fixture::fetch_partition(
...
new_coproc_test_fixture::do_consume_materialized(
  model::ntp ntp,
...
ss::future<model::record_batch_reader::data_t>
new_coproc_test_fixture::consume_materialized(
  model::ntp source,
...
ss::future<> restartable_new_coproc_test_fixture::restart() {
    auto data_dir = _root_fixture->data_dir;
    _root_fixture->remove_on_shutdown = false;
    _root_fixture = nullptr;
    // _root_fixture = std::make_unique<new_coproc_test_fixture>(
    //   std::move(data_dir));
