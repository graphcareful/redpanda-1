...
get_log(storage::log_manager& log_mgr, model::ntp ntp) {
    ...
    co_return co_await log_mgr.manage(
      storage::ntp_config(ntp, log_mgr.config().base_dir));
}

static ss::future<bool> write_materialized_partition(
  model::ntp ntp, model::record_batch_reader reader, output_write_args args) {
    ...
    return found->second.with(
      [ntp, args, reader = std::move(reader)]() mutable {
...
process_one_reply(process_batch_reply::data e, output_write_args args) {
    ...
    if (!e.reader) {
        throw ss::make_exception_future<>(script_failed_exception(
          e.id,
          fmt::format(
