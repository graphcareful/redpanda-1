...
static void push_offsets_ahead(
  std::vector<ss::lw_shared_ptr<ntp_context>> ctxs, script_id id) {
    for (auto ctx : ctxs) {
...
static ss::future<std::tuple<
  ...
  std::vector<ss::lw_shared_ptr<ntp_context>>>>
logs_for_context(
  ...
  absl::flat_hash_map<model::ntp, request_context::state> wgrp) {
    ...
    std::vector<ss::lw_shared_ptr<ntp_context>> ipts;
    /// TODO: Maybe could do this in parallel
    for (auto& [ntp, op] : wgrp) {
        try {
            auto log = co_await get_log(log_mgr, ntp);
            labs.emplace_back(log, std::move(op.bobs));
            ipts.emplace_back(op.ctx);
