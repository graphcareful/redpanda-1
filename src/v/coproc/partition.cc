...
public:
    ...
    do_load_slice(model::timeout_clock::time_point t) final {
        return ss::with_gate(
          _gate, [this, t] { return _impl->do_load_slice(t); });
    }
...
safe_shutdown_appender::operator()(model::record_batch& rb) {
    ...
    return ss::do_with(std::move(rb), [this](model::record_batch& rb) {
        return ss::with_gate(
          _gate, [this, &rb] { return _appender.operator()(rb); });
    });
}
...
partition::make_reader(storage::log_reader_config config) {
    return ss::with_gate(_gate, [this, config] {
...
partition::timequery(storage::timequery_config cfg) {
    return ss::with_gate(_gate, [this, cfg] { return _log.timequery(cfg); });
}
} // namespace coproc
