...
static const model::topic_partition coproc_internal_tp{model::topic_partition(
...
event_listener::poll_topic(model::record_batch_reader::data_t& events) {
    return _client.fetch_partition(coproc_internal_tp, _offset, 64_KiB, 5s)
      .then([this, &events](kafka::fetch_response response) {
          ...
          vassert(p.name == coproc_internal_tp.topic, "Unexpected topic name");
