...
/// In memory cache of all coprocessors of all types
///
...
public:
    ...
    using inverted_lookup_t = absl::
      node_hash_map<model::topic_namespace, absl::node_hash_set<script_id>>;
    ...
    const const_inv_iterator find(const model::topic_namespace& tn) const {
        return _by_input.find(tn);
    }
      ...
      script_id id, iobuf source, std::vector<topic_namespace_policy> inputs) {
        ...
        for (auto& input : inputs) {
            ...
            if (found == _by_input.end()) {
                _by_input.emplace(std::move(input.tn), id);
            } else {
