...
enrich_topics(std::vector<enable_copros_reply::topic_policy> itopics) {
    ...
    std::transform(
      ...
      [](const enable_copros_reply::topic_policy&& tp) {
...
static bool should_immediately_deregister(
  const std::vector<std::vector<coproc::errc>>& codes) {
    ...
    return std::any_of(results.cbegin(), results.cend(), xform::just<bool>());
}
...
ss::future<> script_dispatcher::enable_coprocessors(enable_copros_request req) {
    using reply_t = result<rpc::client_context<enable_copros_reply>>;
    auto client = co_await get_client();
    if (!client) {
        co_return;
    }
    reply_t reply = co_await client->enable_coprocessors(
    ...
    for (enable_copros_reply::data& r : reply.value().data.acks) {
        ...
        auto results = co_await _pacemaker.map(
          [id, itopics = std::move(itopics)](pacemaker& p) {
    ...
    if (!deregisters.empty()) {
        vlog(coproclog.info, "Immediately deregistering ids {}", deregisters);
...
script_dispatcher::disable_coprocessors(disable_copros_request req) {
    using reply_t = result<rpc::client_context<disable_copros_reply>>;
    auto client = co_await get_client();
    if (!client) {
        co_return;
    }
    reply_t reply = co_await client->disable_coprocessors(
      std::move(req), rpc::client_opts(model::no_timeout));
    if (!reply) {
        vlog(
    ...
    for (const auto& [id, code] : reply.value().data.acks) {
        ...
        std::vector<coproc::errc> results = co_await _pacemaker.map(
          [id = id](pacemaker& p) { return p.remove_source(id); });
...
ss::future<> script_dispatcher::disable_all_coprocessors(int) {
    using reply_t = result<rpc::client_context<disable_copros_reply>>;
    struct error_cnt {
        size_t n_success{0};
    ...
    reply_t reply = co_await client->disable_all_coprocessors(
    ...
    error_cnt cnt = std::accumulate(
      ...
      [](error_cnt cnt, const disable_copros_reply::ack& ack) {
          ...
          if (ack.second == disable_response_code::script_id_does_not_exist) {
              cnt.n_script_dnes += 1;
          }
          if (ack.second == disable_response_code::internal_error) {
...
script_dispatcher::get_client() {
    static simple_time_jitter<ss::lowres_clock> jitter{1s};
    while (true) {
        if (_abort_source.abort_requested()) {
            co_return std::nullopt;
        }
        result<rpc::transport*> transport = co_await _transport.get_connected(
          model::no_timeout);
        if (!transport || transport.has_error()) {
            vlog(
              coproclog.error,
              "script_dispatcher failed to aquire a connection to the wasm "
              "engine (to deploy or remove a script), retrying... ");
            co_await ss::sleep(jitter.next_duration());
        } else {
