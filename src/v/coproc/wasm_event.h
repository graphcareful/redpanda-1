...
/// \brief performs the strictest form of validation, 'none' if all checks
/// pass
wasm::errc validate_event(const model::record&);

class event {
public:
    static std::variant<wasm::errc, event> make_event(model::record&& r) {
        auto r = validate_event(r);
        if (r != wasm::errc::none) {
            return std::variant<wasm::errc, event>(r);
        }
        return std::variant<wasm::errc, event>(event(std::move(r)));
    }

    /// \brief returns the id of the event, performs a byte-copy
    script_id get_id() const;

    event_action get_event_action() const {}

    std::string_view get_name() const {
        auto name_itr = get_value_for_event_header(_r, event_header::name);
        return std::string_view{
          name_itr->value().begin(), name_itr->value_size()};
    }

private:
    explicit event(model::record&& r)
      : _r(std::move(r)){};
    ...
    get_value_for_event_header(const model::record&, event_header) const;
    ...
    get_event_action(const model::record&) const;
