...
public:
    script_dispatcher(
      ss::sharded<pacemaker>&, script_database&, ss::abort_source&) noexcept;
...
private:
    ...
    script_database& _sdb;

    /// Reference to the script_listeners abort source. This class uses this
    /// abort source to know when to abort the transports retry loop in order
