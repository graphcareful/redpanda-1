...
service::enable_copro(enable_copros_request::data&& input) {
    ...
    return _pacemaker
      ...
      .then([this, id = input.id, topics = std::move(input.topics)](
          ...
          return _pacemaker
            .map([id, topics = std::move(topics)](pacemaker& pacemaker) {
            ...
            .then([id](std::vector<std::vector<errc>> ack_codes) {
                ...
                for (std::size_t i :
                    ...
                    vassert(
                      ack_codes[i].size() == ack_codes[0].size(),
                      "All vectors should have the same number of "
                      "elements");
    ...
    return ss::do_with(
      ...
      [this](std::vector<enable_copros_request::data>& inputs) {
          return ss::with_scheduling_group(
            get_scheduling_group(), [this, &inputs]() mutable {
