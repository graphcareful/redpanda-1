...
ss::future<> materialized_topics_frontend::create_materialized_topics(
  std::vector<cluster::create_materialized_topic_cmd_data> topics,
  model::timeout_clock::time_point timeout) {
    std::vector<ss::future<>> all;
    std::vector<cluster::create_materialized_topic_cmd_data> todos;
    for (auto& topic : topics) {
        auto [itr, success] = _topics.insert(
          {topic.materialized, ss::make_lw_shared<ss::shared_promise<>>()});
        if (!success) {
    ...
    if (!todos.empty()) {
        auto f = _topics_frontend.local()
                   .create_materialized_topics(std::move(todos), timeout)
