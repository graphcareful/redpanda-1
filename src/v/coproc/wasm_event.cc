...
std::string_view action_as_string_view(wasm_event_action action) {
    static constexpr std::string_view deploy{"deploy"};
...
std::string_view header_as_string_view(wasm_event_header header) {
    ...
    static constexpr std::string_view checksum{"checksum"};
    switch (header) {
...
std::optional<ss::sstring> wasm_event_get_id(const model::record& r) {
    /// Performs a copy
...
wasm_event_get_value_for_header(
  const model::record& r, wasm_event_header header) {
    ...
    for (auto itr = r.headers().cbegin(); itr != r.headers().cend(); ++itr) {
        if (itr->key() == match) {
            return itr;
        }
    }
    return r.headers().cend();
}
...
wasm_event_errc wasm_event_verify_checksum(const model::record& r) {
    ...
    // vlog(coproclog.info, "Checksum {} calculated {}", checksum, calculated);
