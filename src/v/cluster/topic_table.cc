...
topic_table::apply(create_materialized_topic_cmd cmd, model::offset o) {
    ...
    const model::topic_namespace& new_materialized_topic = cmd.key.materialized;
    ...
    if (tp == _topics.end()) {
        ...
        co_return make_error_code(errc::materialized_source_topic_not_exists);
    }
    ...
    tca.source_topic = source.tp;
    tca.configuration.cfg.tp_ns.tp = new_materialized_topic.tp;
    _topics.insert({new_materialized_topic, std::move(tca)});
    const auto found = _topics_hierarchy.find(source);
