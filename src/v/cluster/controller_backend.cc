...
ss::future<bool> controller_backend::insert_to_shard_table(
  model::ntp ntp, ss::shard_id shard, model::revision_id revision) {
    return _shard_table.map_reduce0(
      [self = _self, ntp = std::move(ntp), shard, revision](shard_table& s) {
          vlog(
            clusterlog.trace,
            "[n: {}, r: {}] inserting {} to shard table at {}",
            self,
            revision,
            ntp,
            shard);
          return s.insert(ntp, shard, revision);
      },
      false,
      std::logical_or<>());
}
...
ss::future<std::error_code> controller_backend::create_materialized(
  model::ntp ntp, model::revision_id rev) {
    ...
    if (!cfg->properties.source_topic) {
        // State has been confused, this method should only be called on a topic
        // that has been marked as materialized
