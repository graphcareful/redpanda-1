...
public:
    ...
    bool enqueue_audit_event(audit_event_type type, T&& t) {
...
private:
    ...
    /// Size of bits of representation of enum as bitset is sizeof(uint8_t)
    /// multiplied by number of bits in a byte
    static constexpr auto enabled_set_bitlength
      = 8 * sizeof(std::underlying_type_t<audit_event_type>);
