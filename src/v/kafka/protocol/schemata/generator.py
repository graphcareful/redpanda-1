...
# The following is a list of tag types for which contain fields where their
...
{% macro tag_decoder_impl(tags, obj = "") %}
...
{%- for tag in tags %}
    case {{ tag.tag() }}:
{{- field_decoder(tag, (field_decoder, tag_decoder), obj) | indent | indent }}
        break;
{%- endfor %}
    default:
        /// Read unknown tag
...
{% macro tag_decoder(tags, obj = "") %}
{%- if tags|length == 0 %}
{{- blind_decode() }}
{%- else %}
{
{{- tag_decoder_impl(tags, obj) | indent }}
}
{%- endif %}
{%- endmacro %}

{% macro conditional_tag_encode(tag, vec) %}
{%- if tag.is_array %}
{%- if tag.nullable() %}
{%- call tag_version_guard(tag) %}
if ({{ tag.name }}) {
    {{ vec }}.push_back({{ tag.tag() }});
}
{%- endcall %}
{%- else %}
{%- call tag_version_guard(tag) %}
...
{%- endcall %}
{%- endif %}
{%- elif tag.default_value() != "" %}
{%- call tag_version_guard(tag) %}
if ({{ tag.name }} != {{ tag.default_value() }}) {
    {{ vec }}.push_back({{ tag.tag() }});
}
{%- endcall %}
{%- endif %}
...
{% macro tag_encoder_impl(tags, obj = "") %}
/// Tags encoding section
std::vector<uint32_t> to_encode;
{%- for tag in tags -%}
{{- conditional_tag_encode(tag, "to_encode") }}
{%- endfor %}
...
{%- for t in tags %}
    case {{ t.tag() }}:
{{- field_encoder(t, (field_encoder, tag_encoder), obj, "rw") | indent | indent }}
        break;
{%- endfor %}
    default:
...
{% macro tag_encoder(tags, obj = "") %}
{%- if tags|length == 0 %}
{{- encode_zero() }}
{%- else %}
{
{{- tag_encoder_impl(tags, obj) | indent }}
}
{%- endif %}
{%- endmacro %}
